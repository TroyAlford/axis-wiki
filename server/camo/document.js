...
class Document extends BaseDocument {
  ...
  save() {
    ...
    return Promise.all(preValidatePromises).then(function () {
      ...
      that.validate(); // Ensure all data types are saved in the same encodings
  ...
  static createIndexes() {
    ...
    _.keys(instance._schema).forEach(function (k) {
      if (instance._schema[k].unique) {
        DB().createIndex(that.collectionName(), k, {
          unique: true
        });
      }
    });
  ...
  static _fromData(datas) {
    let instances = super._fromData(datas); // This way we preserve the original structure of the data. Data

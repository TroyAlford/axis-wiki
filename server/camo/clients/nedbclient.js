...
class NeDbClient extends DatabaseClient {
  ...
  save(collection, id, values) {
    ...
    return new Promise(function (resolve, reject) {
      const db = getCollection(collection, that._collections, that._path); // TODO: I'd like to just use update with upsert:true, but I'm
  ...
  findOneAndUpdate(collection, query, values, options) {
    ...
    options.multi = false;
    return new Promise(function (resolve, reject) {
      const db = getCollection(collection, that._collections, that._path); // TODO: Would like to just use 'Collection.update' here, but
      ...
      that.findOne(collection, query).then(function (data) {
        if (!data) {
          ...
          return db.update(query, {
            ...
            if (error) return reject(error); // Fixes issue #55. Remove when NeDB is updated to v1.8+
  ...
  static connect(url, options) {
    ...
    return new Promise(function (resolve, reject) {
      let collections = {}; // TODO: Load all data upfront or on-demand?

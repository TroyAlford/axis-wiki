import {
  difference, flatten, flow, forEach,
  keys, map, startCase, uniq,
  union, xor,
} from 'lodash'
...
class Article {
  ...
  build_html(html) {
    ...
    $html('a').each((index, element) => {
      ...
      if (!this.is_local_url(href)) // External link
        ...
        let link_slug = Slug(link.pathname);
    ...
    return {
      ...
      links_to:      Slug(links_to),
      missing_links: Slug(missing_links)
    };
  }
  ...
  get_rooted_url(value) {
    ...
    let href = Url(value).replace(/^[.]{0,}\//g, '');
